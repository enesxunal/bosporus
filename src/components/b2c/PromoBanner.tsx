    ...
    <section className="relative w-full overflow-hidden bg-bosporus-gray-800">
      <div className="relative h-[280px] md:h-[380px]">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 h-full flex flex-col justify-center">
          <span className="inline-block w-fit px-3 py-1 bg-bosporus-yellow text-bosporus-gray-800 text-xs font-bold uppercase mb-3 rounded-sm">
            {locale === "de" ? "Top-Angebot" : "Fırsat"}
          </span>
          <h2 className="text-3xl md:text-5xl font-bold text-white mb-3 max-w-lg leading-tight">{title}</h2>
          <p className="text-white/90 text-base md:text-lg mb-6 max-w-md">{subtitle}</p>
          <Link
            href={slide.href}
            className="inline-block w-fit px-8 py-3 bg-white text-bosporus-gray-800 font-bold text-sm rounded-sm hover:bg-bosporus-yellow transition-colors"
          >
            {cta} →
          </Link>
        </div>
      ...
      <button
        ...
        className="absolute left-4 top-1/2 -translate-y-1/2 z-20 p-2 bg-white/90 rounded-sm hover:bg-white shadow"
        aria-label="Previous"
      >
      ...
      <button
        ...
        className="absolute right-4 top-1/2 -translate-y-1/2 z-20 p-2 bg-white/90 rounded-sm hover:bg-white shadow"
        aria-label="Next"
      >
      ...
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 z-20 flex gap-2">
        {SLIDES.map((_, i) => (
          <button
            ...
            className={`w-2.5 h-2.5 rounded-full transition-colors ${i === current ? "bg-bosporus-yellow" : "bg-white/50"}`}
          />
        ))}
      </div>
    </section>
